...
@time begin
basis_ref = PlaneWaveBasis(model; Ecut=500, kgrid=kgrid)
scfres_ref = self_consistent_field(basis_ref; tol=tol, callback=identity );
...
for i in 1:length(Ecut_init)
    ...
    f(x) = erreur_force(base_petite, scfres, Ecut, x)[2]
...
@save "data_erreur.jld2" data_courbes_erreur data_courbes_gamma gamma_list f_liste g_liste
