...
function erreur_forces(basis,scfres,Ecut_init,x)
    basis_gamma = PlaneWaveBasis(model; Ecut=Ecut_init*x, kgrid)
   ψr = DFTK.transfer_blochwave(scfres.ψ, basis, basis_gamma)
   ρr = compute_density(basis_gamma, ψr, scfres.occupation)
   Er, hamr = energy_hamiltonian(basis_gamma, ψr, scfres.occupation; ρ=ρr);
   ...
   norm(compute_relerror(f - df_schur))
end
...
for i in 1:length(Ecut_init)
    ...
    f(x) = erreur_forces(base_petite, scfres, Ecut, x)[2]
    ...
    for j in 1:length(Ecut_list)
        er = erreur_forces(base_petite, scfres, Ecut, Ecut_list[j])[2]
...
@save "data_erreur_TiO2.jld2" data_courbes_erreur data_courbes_gamma gamma_list f_liste g_liste

# graphique
plot(xlabel="gamma", ylabel="Erreur de forces", title="Variation de l'erreur de forces en fonction de gamma")
